// SwiftUI shell for the stateful Qwen3-VL 4B generator (text-only).
// Reuses Qwen3VL2BStatefulGenerator (size-agnostic) via Config.default4B,
// loading from Documents/Models/qwen3-vl-4b-stateful/. Runs a 64-token
// smoke test and reports decode tok/s + phys_footprint.
...
struct Qwen3VL4BStatefulGeneratorView: View {
    @State private var gen = Qwen3VL2BStatefulGenerator(cfg: .default4B)
        ...
        NavigationStack {
            Form {
                Section("Stateful generate — text-only") {
                    Text("36-layer Qwen3-VL 4B text backbone via MLState + "
                         + "slice_update (6 chunks, INT4, tied head). Loads from "
                         + "Documents/Models/qwen3-vl-4b-stateful/.")
            ...
            .navigationTitle("VL 4B (stateful)")
        }
    }
    ...
    private func clearChunksDir() {
        ...
        let dirs = [
            docs.appendingPathComponent(
                "Models/qwen3-vl-4b-stateful/qwen3_vl_4b_stateful_chunks"),
            docs.appendingPathComponent("qwen3_vl_4b_stateful_chunks"),
        ]
        var cleared = 0
        for d in dirs {
