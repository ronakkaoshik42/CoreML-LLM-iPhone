#!/usr/bin/env bash
# Compile + sideload the Qwen3-VL 8B stateful chunks to the connected
...
# Documents/Models/qwen3-vl-8b-stateful/qwen3_vl_8b_stateful_chunks/
set -euo pipefail
...
# Optional vision encoder: <SRC_DIR>/../qwen3_vl_8b_vision/vision.mlpackage
# (output of build_qwen3_vl_8b_vision.py). chunk_0_vision.mlpackage lives
# inside SRC_DIR and is already pushed by the *.mlpackage loop above.
VISION_DIR="$(dirname "$SRC_DIR")/qwen3_vl_8b_vision"
VISION_REMOTE="Documents/Models/qwen3-vl-8b-stateful/qwen3_vl_8b_vision"
...
echo "  'Qwen3-VL 8B — text-only' → 'Stateful 64-token smoke test' → Run."
